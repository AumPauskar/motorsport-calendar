...
export default function App() {
  ...
  return <div className="app-shell">
    ...
    <main className="main-content"><header className="topbar"><button className="icon-button menu-toggle" onClick={() => setSidebarOpen(true)} aria-label="Open menu">☰</button><div className="page-nav"><button className={page === 'calendar' ? 'active' : ''} onClick={() => setPage('calendar')}>Calendar</button><button className={page === 'week' ? 'active' : ''} onClick={() => setPage('week')}>This week</button></div><div className="breadcrumb"><span>{page === 'week' ? 'This week' : 'Calendar'}</span><span className="slash">/</span><strong>{series}</strong></div><div className="top-actions"><button className={`theme-toggle ${dark ? 'dark' : ''}`} onClick={() => setDark((value) => !value)} aria-label="Switch color theme"><span>☼</span><span className="theme-track"><i /></span><span>☾</span></button></div></header>
